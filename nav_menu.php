...
<?php
                        ...
                        $parent_count=0;
                        $SQL=" SELECT distinct (a.parentmenu) FROM menu a, group_menu b WHERE a.idmenu=b.idmenu AND b.groupid=$_SESSION[GROUP_USER]  ";
                        ...
                        if(!empty($RS->fields['parentmenu'])){
                                ...
                                while(!$RS->EOF){
                                            ...
                                            $SQL2=" SELECT menuname,icon FROM menu WHERE idmenu='$parent_menu' and parentmenu='0' ";
                                            //echo $SQL2."<br>";
                                            //die();
                                            $RS2  = $db->Execute($SQL2);
                                            #### DISPLAY PARENT
                                            if(!empty($RS2->fields['menuname'])){
                                                ...
                                                $SQL3 =" SELECT distinct (a.menuname), a.idmenu,a.icon  FROM menu a, group_menu b WHERE a.idmenu=b.idmenu ";
                                                $SQL3.=" AND b.groupid=$_SESSION[GROUP_USER]  AND a.parentmenu='$parent_menu' order by a.menuname asc ";
                                                $RS3  = $db->Execute($SQL3);
                                            }
                                                            ...
                                                            $class_submenu="";
                                                             if ($active_submenu == sha1($RS3->fields['idmenu']) && $active_submenu!="") {
                                                                ...
                                                                $myfont="font-yellow";
                                                                 ...
                                                                 echo "<li  ><a href='home?module=".sha1($RS3->fields['idmenu'])."&pm=".sha1($parent_menu)."'><button class='btn $default' > <i class='".$RS3->fields['icon']." $myfont'></i> <span class='text-uppercase title bold $myfont'> ".$RS3->fields['menuname']."</span> </button></a></li>";
