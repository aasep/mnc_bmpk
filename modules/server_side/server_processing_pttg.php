...
/*if (isset($_GET['act1']) ) {
    ...
    $aColumns = array('no','cfsnme','cpgdes', 'cpplaf','cpouts', 'status','action' );
	$aColumns_sorting =array('cfclas','cfsnme','cpgdes', 'cpplaf','cpouts', 'status','status' );
    ...
    $aColumnsSearch= array('cfsnme','cpgdes', 'status' );
    if ( $_GET['sSearch'] != "" )
    {
        $sWhere = "WHERE (";
...
*/
    ...
    $sQuery  = " select cfclas,cfsnme,cpgdes, cpplaf,cpouts, status,cfcif  from pihak_tdk_terkait  ";
    $sQuery .=" $sWhere ".
    ...
    $sQuery2  =" select count(*) as jumlah  from pihak_tdk_terkait ";
    $sQuery2 .=" $sWhere ";
    //$sQuery .=" where a.username='".getUsername()."' ";
   //echo $sQuery2."<br>";
   // die();
    ...
    while(!$rResult->EOF)
    {
        ...
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
            ...
            {
               $row[] = "<div > <b>".$rResult->fields['cfsnme']." </b></div>"; ;
            }
        
            else if ($aColumns[$i] == "cpgdes") 
            {   
               $row[] = "<div ><b>".$rResult->fields['cpgdes'] ."</b></div>";
            }
            ...
            {  
               $row[] = "<div align='right'> <b>".number_format($rResult->fields['cpouts'],2,".",",")."</b></div>"; 
            }
            else if ($aColumns[$i] == "status") 
            {  
               $row[] = "<div align='right'> <b>".$rResult->fields['status']."</b></div>"; 
            }
